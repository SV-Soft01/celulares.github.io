// Sistema de sincronización optimizado con Firebase
// Implementa sincronización incremental y manejo de usuarios

;(() => {
    console.log("Inicializando sincronización Firebase optimizada...")
    ...
    function interceptarFuncionesUsuarios() {
      ...
      if (typeof window.iniciarSesion === "function") {
        var iniciarSesionOriginal = window.iniciarSesion
        window.iniciarSesion = function () {
          var resultado = iniciarSesionOriginal.apply(this, arguments)
      ...
      if (typeof window.registrarUsuario === "function") {
        var registrarUsuarioOriginal = window.registrarUsuario
        window.registrarUsuario = function () {
          var resultado = registrarUsuarioOriginal.apply(this, arguments)
      ...
      if (typeof window.guardarUsuarios === "function") {
        var guardarUsuariosOriginal = window.guardarUsuarios
        window.guardarUsuarios = function () {
          var resultado = guardarUsuariosOriginal.apply(this, arguments)
    ...
    function crearPanelDiagnostico() {
      ...
      panel.innerHTML = `
        ...
        <div id="estado-intercepcion" style="margin-bottom: 10px;">
          <span style="display: inline-block; width: 10px; height: 10px; border-radius: 50%; background-color: #FFC107; margin-right: 5px;"></span>
          <span>Intercepción: Verificando...</span>
        </div>
        <div id="estado-usuarios" style="margin-bottom: 15px;">
          <span style="display: inline-block; width: 10px; height: 10px; border-radius: 50%; background-color: #FFC107; margin-right: 5px;"></span>
          <span>Usuarios: Verificando...</span>
        </div>
        <div style="display: flex; gap: 5px; margin-bottom: 10px;">
          ...
          <button id="btn-usuarios" style="flex: 1; padding: 8px; background-color: #28a745; color: white; border: none; border-radius: 3px; cursor: pointer;">Sincronizar Usuarios</button>
        </div>
    ...
    function actualizarEstadoDiagnostico(estado) {
      ...
      var estadoUsuarios = document.getElementById("estado-usuarios")
  
      if (!estadoFirebase || !estadoFirestore || !estadoIntercepcion || !estadoUsuarios) return
      ...
      // Verificar usuarios
      var hayUsuarios = localStorage.getItem("usuarios") !== null
      estadoUsuarios.innerHTML = `
        <span style="display: inline-block; width: 10px; height: 10px; border-radius: 50%; background-color: ${hayUsuarios ? "#4CAF50" : "#FFC107"}; margin-right: 5px;"></span>
        <span>Usuarios: ${hayUsuarios ? "Disponibles" : "No encontrados"}</span>
      `
    ...
    function reiniciarSincronizacion() {
      ...
      cargarDatosDesdeFirebase(false).then(() => {
        // Después cargar usuarios
